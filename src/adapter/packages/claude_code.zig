...
pub const package: types.AdapterPackage = .{
    ...
    .repo_scope_description = "Only this repository (.claude + .mcp.json)",
    .user_scope_description = "All Claude Code sessions on this machine (~/.claude)",
    .remove_repo_scope_description = "Current repository install (.claude + .mcp.json)",
...
fn renderSessionStartHook(
    ...
    const workflow_skills_dir = switch (scope) {
        .repo => try std.fs.path.join(allocator, &.{ target_root, ".claude", "skills" }),
...
fn scopedRelativePath(
    ...
    return switch (scope) {
        .repo => switchKeyToPath(allocator, ".claude", resource_key),
...
fn commandJsonLiteral(
    ...
    const script_path = switch (scope) {
        .repo => try std.fs.path.join(allocator, &.{ target_root, ".claude", "hooks", script_name }),
...
test "renderRuntimeAssets uses repo-local Claude Code hook paths" {
    const allocator = std.testing.allocator;
    const assets = try renderRuntimeAssets(allocator, .repo, "/tmp/project");
    defer deinitRenderedAssets(allocator, assets);

    try std.testing.expect(std.mem.indexOf(u8, assets[0].content, "/tmp/project/.claude/hooks/session-start.sh") != null);
}
