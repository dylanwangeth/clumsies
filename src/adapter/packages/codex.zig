...
pub const package: types.AdapterPackage = .{
    ...
    .repo_scope_description = "Only this repository (.codex + .agents/skills)",
    .user_scope_description = "All Codex sessions on this machine (~/.codex)",
    .remove_repo_scope_description = "Current repository install (.codex)",
...
pub fn renderRuntimeAssets(
    ...
    if (scope == .repo) {
        const project_root = repoProjectRoot(target_root);
        const skills_root_absolute = try std.fs.path.join(allocator, &.{ project_root, ".agents", "skills" });
...
pub fn resolveTargetRoot(
    ...
    return switch (scope) {
        .repo => blk: {
            const repo_root = repo_root_opt orelse break :blk null;
            break :blk try std.fs.path.join(allocator, &.{ repo_root, ".codex" });
        },
...
fn renderSessionStartHook(
    ...
    const workflow_skills_dir = switch (scope) {
        .repo => blk: {
            const project_root = repoProjectRoot(target_root);
            break :blk try std.fs.path.join(allocator, &.{ project_root, ".agents", "skills" });
        },
...
fn repoProjectRoot(target_root: []const u8) []const u8 {
...
test "renderRuntimeAssets uses absolute codex hook paths" {
    const allocator = std.testing.allocator;
    const assets = try renderRuntimeAssets(allocator, .repo, "/tmp/project/.codex");
    defer deinitRenderedAssets(allocator, assets);

    try std.testing.expect(std.mem.indexOf(u8, assets[1].content, "/tmp/project/.codex/hooks/session-start.sh") != null);
}
