...
pub fn resolveRepoRoot(allocator: std.mem.Allocator) !?[]const u8 {
    ...
    var output: git.GitOutput = .{};
    defer output.deinit(allocator);
    git.run(allocator, cwd, &.{ "git", "rev-parse", "--show-toplevel" }, &output) catch {
        allocator.free(cwd);
        return null;
    };

    const stdout = output.stdout orelse {
        allocator.free(cwd);
        return null;
    };
    var end = stdout.len;
    while (end > 0 and (stdout[end - 1] == '\n' or stdout[end - 1] == '\r')) : (end -= 1) {}
    const root = try allocator.dupe(u8, stdout[0..end]);
    allocator.free(cwd);
    return root;
}
...
pub fn buildAdaptPlan(
    ...
    const computed_install_id = try store.installIdForTarget(allocator, pkg.id, scope.asString(), target_root);
    var loaded_opt = try store.loadManifestForTarget(allocator, pkg.id, scope.asString(), target_root);
