...
pub fn fingerprintForContent(allocator: std.mem.Allocator, content: []const u8) ![]const u8 {
    const hash = @import("../protocol/hash.zig").ContentHash.compute(content);
...
test "installIdForTarget differs by target root" {
    const a = try installIdForTarget(std.testing.allocator, "codex", "repo", "/tmp/a");
    defer std.testing.allocator.free(a);
    const b = try installIdForTarget(std.testing.allocator, "codex", "repo", "/tmp/b");
    defer std.testing.allocator.free(b);
    try std.testing.expect(!std.mem.eql(u8, a, b));
}

test "installIdForTarget differs by scope" {
    const repo = try installIdForTarget(std.testing.allocator, "codex", "repo", "/tmp/a");
    defer std.testing.allocator.free(repo);
    const user = try installIdForTarget(std.testing.allocator, "codex", "user", "/tmp/a");
    defer std.testing.allocator.free(user);
    try std.testing.expect(!std.mem.eql(u8, repo, user));
}
