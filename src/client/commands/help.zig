...
pub fn run(stdout: *std.Io.Writer) !void {
    try stdout.print("{s}{s}{s}clumsies{s} - Building the persistent, observable, and collaborative context infrastructure that coexists with agents' self-managed memory.\n\n", .{ P, Color.bold, Color.orange, Color.reset });
    try stdout.print("{s}{s}{s}Usage:{s}\n", .{ P, Color.bold, Color.orange, Color.reset });
    try stdout.print("{s}    {s}clumsies{s}            Launch TUI Dashboard\n", .{ P, Color.cyan, Color.reset });
    try stdout.print("{s}    {s}clumsies login{s}      Authenticate with Server\n", .{ P, Color.cyan, Color.reset });
    try stdout.print("{s}    {s}clumsies init{s}       Bind workspace to current directory\n", .{ P, Color.cyan, Color.reset });
    try stdout.print("{s}    {s}clumsies sync{s}         Sync local cache from Server\n", .{ P, Color.cyan, Color.reset });
