...
fn buildInitializeResult(allocator: std.mem.Allocator, version: []const u8) ![]u8 {
    ...
    const instructions =
        ...
        "Use " ++ tool_names.kanban ++ " for the native Kanban (distinct from remote GitHub Issues). Other agents share this board: before creating or mutating anything, call kanban.list first to check for existing Issues and active runs, so you do not duplicate work or claim an Issue another agent is already working on. Then call kanban.get when the user supplies a global issue_id, create durable Todo work only after the list check, update semantic content, call kanban.begin_work after deciding the prompt starts or continues an Issue (skip it if the Issue is already bound to an active run), and call kanban.request_closure only after judging its acceptance criteria satisfied. kanban.get returns the owning project_id; mutations remain scoped to this MCP workspace. Agents cannot approve closure. AgentRun lifecycle events never advance an Issue.";
