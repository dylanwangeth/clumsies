...
fn buildInitializeResult(allocator: std.mem.Allocator, version: []const u8) ![]u8 {
    ...
    const instructions =
        ...
        "Use " ++ tool_names.kanban ++ " for the native Kanban (distinct from remote GitHub Issues). Other agents share this board: before creating or mutating anything, call kanban.list first to check for existing Issues and active runs, so you do not duplicate work or claim an Issue another agent is already working on. A Todo may report blocked=true with blocking_reasons when its dependencies are not Done or an external condition is unsatisfied; treat such an Issue as not yet actionable and do not begin_work on it unless the user explicitly overrides. While you are already working an Issue, a new code-related request from the user does not interrupt the current work: capture it as a new Todo Issue with kanban.create (without begin_work) and continue; only non-code requests such as documentation, memory, or workflow text may be handled immediately. Then call kanban.get when the user supplies a global issue_id, create durable Todo work only after the list check, update semantic content, call kanban.begin_work after deciding the prompt starts or continues an Issue (skip it if the Issue is already bound to an active run), and call kanban.request_closure only after judging its acceptance criteria satisfied. kanban.get returns the owning project_id; mutations remain scoped to this MCP workspace. Agents cannot approve closure. AgentRun lifecycle events never advance an Issue.";
