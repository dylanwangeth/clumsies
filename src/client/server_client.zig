//! HTTP client for the Server server REST API. Handles Bearer token auth, content-type headers,
...
const log = std.log.scoped(.server_client);
...
/// Failures are swallowed inside ServerClient —
...
pub const ServerClient = struct {
    ...
    // these null and ServerClient behaves identically to the pre-refresh
    ...
    pub fn init(allocator: std.mem.Allocator, server_url: []const u8, access_token: ?[]const u8) ServerClient {
        return .{
            .allocator = allocator,
    ...
    pub fn deinit(self: *ServerClient) void {
    ...
    pub fn get(self: *ServerClient, path: []const u8) !Response {
        return self.doFetch(.GET, path, null);
    }

    pub fn post(self: *ServerClient, path: []const u8, body: []const u8) !Response {
        return self.doFetch(.POST, path, body);
    }

    pub fn put(self: *ServerClient, path: []const u8, body: []const u8) !Response {
        return self.doFetch(.PUT, path, body);
    }

    pub fn delete(self: *ServerClient, path: []const u8) !Response {
        return self.doFetch(.DELETE, path, null);
    }

    pub fn patch(self: *ServerClient, path: []const u8, body: []const u8) !Response {
        return self.doFetch(.PATCH, path, body);
    }

    fn effectiveToken(self: *const ServerClient) ?[]const u8 {
        return self.access_token_rotated orelse self.access_token;
    }

    pub fn currentAccessToken(self: *const ServerClient) ?[]const u8 {
        return self.effectiveToken();
    }

    pub fn currentRefreshToken(self: *const ServerClient) ?[]const u8 {
        return self.refresh_token;
    }

    fn doFetch(self: *ServerClient, method: http.Method, path: []const u8, payload: ?[]const u8) !Response {
    ...
    fn refreshAndPersist(self: *ServerClient) !void {
        ...
        persist(self.allocator, self.server_url, un, parsed.value.access_token, parsed.value.refresh_token) catch {};
    }

    fn doFetchOnce(self: *ServerClient, method: http.Method, path: []const u8, payload: ?[]const u8) !Response {
