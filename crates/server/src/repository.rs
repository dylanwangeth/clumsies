...
impl ServerRepository {
    ...
    pub async fn create_org(&self, name: &str) -> Result<String, ServerError> {
        ...
        sqlx::query("INSERT INTO orgs (org_id, name) VALUES ($1, $2)")
            ...
            .execute(&mut *tx)
            .await?;
        insert_ref(&mut tx, "org", &org_id, None).await?;
        tx.commit().await?;
        Ok(org_id)
    }
    ...
    pub async fn bootstrap_self_hosted(
        ...
        project_name: &str,
    ) -> Result<BootstrapIdentity, ServerError> {
        ...
        {
            Some(org_id) => org_id,
            None => self.create_org(org_name).await?,
        };
        ...
        .await?
        {
            ...
            None => {
                self.create_user(owner_email, owner_display_name, "owner")
                    .await?
            }
        };
        let project_id = match sqlx::query_scalar::<_, String>(
            "SELECT project_id FROM projects WHERE org_id = $1 ORDER BY created_at LIMIT 1",
        )
        .bind(&org_id)
        .fetch_optional(&self.pool)
        .await?
        {
            ...
            None => self.create_project(&org_id, project_name, "").await?,
        };
        sqlx::query(
    ...
    pub async fn create_review_merge(
        ...
        let row = sqlx::query(
            "SELECT r.review_id, r.draft_id, r.project_id, r.status, r.version,
                    d.resource_scope, d.base_commit_id
             FROM reviews r
        ...
        if current_head.as_deref() != expected_project_ref {
            return Err(ServerError::ref_conflict(
...
pub enum ServerError {
    ...
    #[error("ref conflict: expected {expected:?}, actual {actual:?}")]
    RefConflict {
...
impl ServerError {
    ...
    fn ref_conflict(expected: Option<&str>, actual: Option<&str>) -> Self {
        Self::RefConflict {
...
async fn load_draft_operations(
    ...
    let rows = sqlx::query(
        "SELECT operation_id, action, resource_scope, resource_kind, target_id, path, base_hash,
    ...
    for operation in operations.iter().skip(1) {
        ...
        match operation.input.action {
            DraftOperationAction::Create => {
                ...
                materialized.body = operation.input.body.clone();
            }
    ...
    match operation.action {
        ...
        DraftOperationAction::Update => {
            ...
            let body = operation.body.as_deref().ok_or_else(|| {
                ServerError::InvalidRequest("update operation requires body".to_owned())
            })?;
            sqlx::query(
                "UPDATE resources
...
async fn apply_metaprompt_operation(
    ...
    match operation.action {
        DraftOperationAction::Create | DraftOperationAction::Update => {
            let body = operation.body.as_deref().ok_or_else(|| {
                ServerError::InvalidRequest("metaprompt create/update requires body".to_owned())
            })?;
...
async fn create_project_commit(
    ...
    let project_rows = sqlx::query(
        "SELECT resource_id, resource_kind, path, body
         FROM resources
    ...
    let selected_rows = sqlx::query(
        "SELECT r.resource_id, r.resource_kind, r.path, r.body
...
async fn pending_resource_entry(
    ...
    Ok(PendingTreeEntry {
        item_id: row.try_get("resource_id")?,
...
fn assert_base_hash(resource: &TargetResource, base_hash: Option<&str>) -> Result<(), ServerError> {
    match base_hash {
        Some(base_hash) if base_hash == resource.content_hash => Ok(()),
        Some(base_hash) => Err(ServerError::InvalidRequest(format!(
