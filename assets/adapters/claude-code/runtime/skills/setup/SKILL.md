...
user-invocable: true
---
Call the `retrieve` MCP tool once for the current host session to bootstrap
...
After loading, briefly summarize the key protocol rules (activate → retrieve → store cycle and the priority model) to confirm the bootstrap succeeded.
