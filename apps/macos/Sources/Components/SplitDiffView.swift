...
struct SplitDiffView: View {
    ...
    init(
        ...
        modifiedPath: String? = nil
    ) {
        model = .make(original: original, modified: modified)
        self.originalTitle = originalTitle
        self.modifiedTitle = modifiedTitle
        self.originalPath = originalPath
        self.modifiedPath = modifiedPath
    }

    var body: some View {
        GeometryReader { proxy in
            let contentWidth = max(760, proxy.size.width)
            ScrollView([.horizontal, .vertical]) {
                LazyVStack(spacing: 0, pinnedViews: [.sectionHeaders]) {
                    Section {
                        if model.blocks.isEmpty {
                            noChanges(width: contentWidth)
                        } else {
                            ForEach(model.blocks) { block in
                                blockView(block, width: contentWidth)
                            }
                        }
                    } header: {
                        columnHeader(width: contentWidth)
                    }
                }
    ...
    private func columnHeader(width: CGFloat) -> some View {
        let paneWidth = (width - 1) / 2
        return VStack(spacing: 0) {
            HStack(spacing: 0) {
                paneHeader(title: originalTitle, path: displayedOriginalPath)
                    .frame(width: paneWidth, alignment: .leading)
                Divider()
                paneHeader(title: modifiedTitle, path: displayedModifiedPath)
                    .frame(width: paneWidth, alignment: .leading)
            }
            .frame(height: showsPathChange ? 42 : 34)
            Divider()
        }
        .background(Color(nsColor: .textBackgroundColor))
    }
    ...
    @ViewBuilder
    private func blockView(_ block: SplitDiffBlock, width: CGFloat) -> some View {
        switch block.kind {
        ...
        case .omission:
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: expandedOmissionIds.contains(block.id)
                        ? "chevron.up"
                        : "chevron.down")
                    Text("\(block.rows.count) unchanged lines")
                }
                .font(.caption)
                .foregroundStyle(.secondary)
                .padding(.horizontal, 10)
    ...
    private func diffRow(_ row: SplitDiffRow, width: CGFloat) -> some View {
        let paneWidth = (width - 1) / 2
        return HStack(alignment: .top, spacing: 0) {
            diffCell(row.original)
                .frame(width: paneWidth, alignment: .topLeading)
            Divider()
            diffCell(row.modified)
                .frame(width: paneWidth, alignment: .topLeading)
        }
        .fixedSize(horizontal: false, vertical: true)
    }

    private func diffCell(_ cell: SplitDiffCell?) -> some View {
        HStack(alignment: .top, spacing: 0) {
            Text(cell.map { String($0.lineNumber) } ?? "")
                ...
                .background(gutterBackground(for: cell?.kind))

            Text(prefix(for: cell?.kind))
                .foregroundStyle(foreground(for: cell?.kind))
                .frame(width: 20, alignment: .center)

            Text(displayText(for: cell))
    ...
    private func noChanges(width: CGFloat) -> some View {
    ...
    private var showsPathChange: Bool {
        originalPath != modifiedPath && (originalPath != nil || modifiedPath != nil)
    }

    private var displayedOriginalPath: String? {
        showsPathChange ? originalPath ?? "/dev/null" : nil
    }

    private var displayedModifiedPath: String? {
        showsPathChange ? modifiedPath ?? "/dev/null" : nil
    }

    private func prefix(for kind: SplitDiffCellKind?) -> String {
        switch kind {
        case .insertion: "+"
        case .removal: "−"
        case .context, nil: ""
    ...
    private func displayText(for cell: SplitDiffCell?) -> String {
        guard let cell, !cell.text.isEmpty else { return " " }
        return cell.text
    }

    private func foreground(for kind: SplitDiffCellKind?) -> Color {
        switch kind {
        case .insertion: .green
        case .removal: .red
        case .context, nil: .secondary
        }
    }

    private func background(for kind: SplitDiffCellKind?) -> Color {
        switch kind {
        case .insertion: .green.opacity(0.10)
        case .removal: .red.opacity(0.10)
        case .context, nil: .clear
        }
    }

    private func gutterBackground(for kind: SplitDiffCellKind?) -> Color {
        switch kind {
        case .insertion: .green.opacity(0.16)
        case .removal: .red.opacity(0.16)
        case .context: Color(nsColor: .controlBackgroundColor).opacity(0.42)
        case nil: .clear
        }
    }
}
