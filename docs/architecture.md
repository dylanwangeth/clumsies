...
Current docs should keep one implementation fact explicit: the running tool surface is `memory.*`. Public docs should describe that as the real protocol surface rather than as a temporary alias for some other namespace.
...
- MCP uses the `memory.*` surface in the current implementation
