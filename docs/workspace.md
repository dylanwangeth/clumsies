...
- [Organization Memory](/artifact)：唯一内容权威与 Bundle；
- [统一 Memory 数据模型](/unified-memory-model)：字段、投影、Draft 与多 Draft Review；
- [本地运行时](/runtime)：目录解析、存储、同步和失败行为；
- [Adapter](/adapter)：各 Agent Host 的纳管方式；
- [系统架构](/architecture)：跨组件不变量。
